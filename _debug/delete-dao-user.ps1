...
$targetCountsSql = "SELECT 'users' AS table_name, COUNT(*) AS remaining FROM users WHERE email_lower = '$escapedEmail' UNION ALL SELECT 'checkout_intents', COUNT(*) FROM checkout_intents WHERE lower(checkout_email) = '$escapedEmail' UNION ALL SELECT 'consent_log', COUNT(*) FROM consent_log WHERE lower(checkout_email) = '$escapedEmail' OR lower(COALESCE(session_email, '')) = '$escapedEmail' UNION ALL SELECT 'magic_login_tokens', COUNT(*) FROM magic_login_tokens WHERE email_lower = '$escapedEmail' UNION ALL SELECT 'email_otp_active', COUNT(*) FROM email_otp_active WHERE email_lower = '$escapedEmail' UNION ALL SELECT 'email_otp_tokens', COUNT(*) FROM email_otp_tokens WHERE email_lower = '$escapedEmail' UNION ALL SELECT 'webhook_events_payload', COUNT(*) FROM webhook_events WHERE lower(payload_json) LIKE '%$escapedEmail%';"
